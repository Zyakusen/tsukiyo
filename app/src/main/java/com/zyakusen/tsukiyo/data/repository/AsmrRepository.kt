package com.zyakusen.tsukiyo.data.repository

import com.zyakusen.tsukiyo.data.AuthManager
import com.zyakusen.tsukiyo.data.SettingsStore
import com.zyakusen.tsukiyo.data.api.AsmrApi
import com.zyakusen.tsukiyo.data.model.AuthResponse
import com.zyakusen.tsukiyo.data.model.Circle
import com.zyakusen.tsukiyo.data.model.Playlist
import com.zyakusen.tsukiyo.data.model.PlaylistsResponse
import com.zyakusen.tsukiyo.data.model.Tag
import com.zyakusen.tsukiyo.data.model.Track
import com.zyakusen.tsukiyo.data.model.Va
import com.zyakusen.tsukiyo.data.model.Work
import com.zyakusen.tsukiyo.data.model.WorksResponse
...
class AsmrRepository(
    ...
    suspend fun guestLogin(): Boolean = runCatching {
        val resp = api.login(com.zyakusen.tsukiyo.data.model.LoginRequest("guest", "guest"))
    ...
    suspend fun login(name: String, password: String, recommenderUuid: String? = null): Result<AuthResponse> =
        runCatching {
            val resp = api.login(com.zyakusen.tsukiyo.data.model.LoginRequest(name, password, recommenderUuid))
            authManager.save(resp.user, resp.token)
            resp
        }
    ...
    suspend fun register(name: String, password: String, recommenderUuid: String? = null): Result<AuthResponse> =
        runCatching {
            val resp = api.register(com.zyakusen.tsukiyo.data.model.LoginRequest(name, password, recommenderUuid))
            authManager.save(resp.user, resp.token)
            resp
        }
    ...
    suspend fun rateWork(workId: Long, rating: Double?, reviewText: String? = null) {
        api.putReview(com.zyakusen.tsukiyo.data.model.ReviewRequest(workId, rating, reviewText, null))
    }

    suspend fun markProgress(workId: Long, progress: String) {
        api.putReview(com.zyakusen.tsukiyo.data.model.ReviewRequest(workId, null, null, progress))
    }
    ...
    suspend fun voteTag(workId: Long, tagId: Long, status: Int) {
        api.voteTag(com.zyakusen.tsukiyo.data.model.VoteRequest(workId, tagId, status))
    }

    // ---------- 推荐 ----------

    suspend fun popularWorks(page: Int = 1, pageSize: Int = 20): WorksResponse =
        api.popular(com.zyakusen.tsukiyo.data.model.RecommenderRequest(page = page, pageSize = pageSize))

    suspend fun recommendWorks(page: Int = 1, pageSize: Int = 20): WorksResponse =
        api.recommendForUser(com.zyakusen.tsukiyo.data.model.RecommenderRequest(page = page, pageSize = pageSize))

    suspend fun similarWorks(itemId: Long): WorksResponse =
        api.itemNeighbors(com.zyakusen.tsukiyo.data.model.ItemNeighborsRequest(itemId = itemId))
    ...
    suspend fun createPlaylist(name: String, privacy: Int = 0, description: String = ""): Playlist =
        api.createPlaylist(
            com.zyakusen.tsukiyo.data.model.CreatePlaylistRequest(
                name = name,
                privacy = privacy,
                locale = "zh-cn",
    ...
    suspend fun addWorksToPlaylist(playlistId: String, workIds: List<Long>) {
        api.addWorksToPlaylist(com.zyakusen.tsukiyo.data.model.PlaylistWorksRequest(playlistId, workIds))
    }

    suspend fun removeWorksFromPlaylist(playlistId: String, workIds: List<Long>) {
        api.removeWorksFromPlaylist(com.zyakusen.tsukiyo.data.model.PlaylistWorksRequest(playlistId, workIds))
    }
    ...
    suspend fun editPlaylist(playlistId: String, name: String, privacy: Int, description: String) {
        api.editPlaylistMetadata(
            com.zyakusen.tsukiyo.data.model.EditPlaylistRequest(
                playlistId,
                com.zyakusen.tsukiyo.data.model.EditPlaylistData(name, privacy, description)
            )
        )
    }
